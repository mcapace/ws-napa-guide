    ...
    <div style={{ minHeight: '100vh' }}>
      ...
      <div style={{ background: '#0D0B09', color: '#F7F3EC' }}>
        ...
        <section style={{ padding: '48px 60px 32px', maxWidth: 960, margin: '0 auto', textAlign: 'center' }}>
          ...
          {place.address && (
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: '#9B9283', marginBottom: 28 }}>{place.address}</p>
          )}
          <div style={{ display: 'flex', gap: 16, alignItems: 'center', justifyContent: 'center', flexWrap: 'wrap', marginBottom: 48 }}>
            {place.website && (
              <a href={place.website} target="_blank" rel="noopener noreferrer" style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 10, fontWeight: 500, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#0D0B09', background: '#F7F3EC', padding: '12px 24px', textDecoration: 'none' }}>
                Book your stay
              </a>
            )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, alignItems: 'start' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', height: '100%' }}>
              <div style={{ borderTop: '1px solid rgba(13,11,9,0.1)', paddingTop: 32, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 32 }}>
                <div>
                  <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 9, fontWeight: 500, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#C4943A', marginBottom: 8 }}>Category</p>
                  <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 300, color: 'rgba(13,11,9,0.6)' }}>{cat}</p>
                </div>
                <div>
                  ...
                  <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 300, color: 'rgba(13,11,9,0.6)' }}>{place.priceRange}</p>
                </div>
                {place.address && (
                  <div style={{ gridColumn: '1 / -1' }}>
                    ...
                    <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 300, color: 'rgba(13,11,9,0.6)' }}>{place.address}</p>
                  </div>
                )}
              </div>
              <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                {place.website && (
                  <a href={place.website} target="_blank" rel="noopener noreferrer" style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 10, fontWeight: 500, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#F7F3EC', background: '#0D0B09', padding: '12px 24px', textDecoration: 'none' }}>
                    Book your stay
                  </a>
                )}
                <Link href={`/map?region=${place.region}`} style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#6B1C2A', textDecoration: 'none', padding: '12px 24px', border: '1px solid rgba(13,11,9,0.15)' }}>

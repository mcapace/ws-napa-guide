...
const categoryLabel: Record<string, string> = { resort: 'Resort', boutique: 'Boutique hotel', inn: 'Country inn', villa: 'Private villa' }
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!h) return {}
  return { title: `${h.name} — Where to Stay in Napa`, description: h.excerpt, openGraph: { images: h.images } }
}
    ...
    <div style={{ minHeight: '100vh', background: '#FFFFFF' }}>
      ...
      <section style={{ textAlign: 'center', padding: '24px 24px 28px', borderBottom: '1px solid #E8E4DE', background: '#FFFFFF' }}>
        <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 10, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.12em', color: '#999', margin: '0 0 4px' }}>
          {cat} · {place.priceRange} · {regionName}
        </p>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 10 }}>
          {place.website && (
            <a href={place.website} target="_blank" rel="noopener noreferrer" style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#FFFFFF', background: '#0D0B09', padding: '12px 24px', textDecoration: 'none' }}>
              Book Your Stay
            </a>
          )}
      ...
      <section id="story" style={{ padding: '0 clamp(24px, 6vw, 100px) 64px', background: '#FFFFFF', maxWidth: 1100, marginLeft: 'auto', marginRight: 'auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40, marginBottom: 48, alignItems: 'start' }}>
          ...
          <div style={{ position: 'relative', aspectRatio: '3/4', overflow: 'hidden' }}>
            <Image src={place.images[1 % place.images.length]} alt="" fill sizes="50vw" style={{ objectFit: 'cover' }} />
          </div>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40, marginBottom: 48, alignItems: 'start' }}>
          <div style={{ position: 'relative', aspectRatio: '4/3', overflow: 'hidden' }}>
            <Image src={place.images[2 % place.images.length]} alt="" fill sizes="50vw" style={{ objectFit: 'cover' }} />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', height: '100%' }}>
            <div style={{ borderTop: '1px solid rgba(0,0,0,0.08)', paddingTop: 28, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 28 }}>
              <div>
                <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 9, fontWeight: 600, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#C4943A', marginBottom: 6 }}>Category</p>
                <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 300, color: 'rgba(0,0,0,0.5)' }}>{cat}</p>
              </div>
              <div>
                ...
                <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 300, color: 'rgba(0,0,0,0.5)' }}>{place.priceRange}</p>
              </div>
              {place.address && (
                <div style={{ gridColumn: '1 / -1' }}>
                  ...
                  <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 300, color: 'rgba(0,0,0,0.5)' }}>{place.address}</p>
                </div>
              )}
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              {place.website && (
                <a href={place.website} target="_blank" rel="noopener noreferrer" style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#FFFFFF', background: '#0D0B09', padding: '13px 24px', textDecoration: 'none' }}>
                  Book Your Stay
                </a>
              )}
        ...
        <section style={{ background: '#F5F3EE', padding: '56px clamp(24px, 5vw, 80px)' }}>
          <div style={{ maxWidth: 1100, margin: '0 auto' }}>
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.12em', color: '#999', margin: '0 0 8px' }}>
              ◆ More places to stay in {regionName}
            </p>
            {related.slice(0, 8).map((h) => (
              <div key={h.slug} style={{ display: 'grid', gridTemplateColumns: '120px 200px 1fr auto', gap: 24, alignItems: 'center', padding: '22px 0', borderBottom: '1px solid #DEDAD4' }}>
                <div style={{ position: 'relative', width: 120, height: 80, overflow: 'hidden' }}>
                  <Image src={h.images[0]} alt={h.name} fill sizes="120px" style={{ objectFit: 'cover' }} />
                </div>
                <div>
                  <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 18, fontStyle: 'italic', fontWeight: 400, color: '#0D0B09', margin: '0 0 3px', lineHeight: 1.2 }}>{h.name}</p>
                  <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#999', margin: 0 }}>{categoryLabel[h.category] ?? h.category} · {h.priceRange}</p>
                </div>
                <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, lineHeight: 1.55, color: '#777', margin: 0, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' as const }}>{h.excerpt}</p>
                <div style={{ display: 'flex', gap: 8 }}>
                  <Link href={`/stay/${h.slug}`} style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 9, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#0D0B09', background: '#F7F3EC', padding: '10px 16px', textDecoration: 'none', border: '1px solid #0D0B09' }}>Book Now</Link>
                  <Link href={`/stay/${h.slug}`} style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 9, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#0D0B09', padding: '10px 16px', textDecoration: 'none', border: '1px solid #CCC' }}>Read More</Link>
                </div>
              </div>
            ))}
          </div>
